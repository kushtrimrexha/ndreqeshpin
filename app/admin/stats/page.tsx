...
export default async function AdminStatsPage() {
  ...
  const { data: profile } = await supabase
    .from('profiles').select('id, role, full_name, package_type').eq('id', session.user.id).single()
  ...
  const [
    ...
    newUsersMonth, newAppsMonth, newOffersWeek, premiumUsers,
    recentUsers, recentApps, monthlyData, cityData,
  ] = await Promise.all([
    ...
    safeList(supabaseAdmin.from('profiles').select('city').not('city','is',null).limit(500)),
  ])
  ...
  cityData.forEach((u:any) => {
    if (u.city) cities[u.city] = (cities[u.city]||0) + 1
  })

  return (
    <PageShell role="admin" userName={profile.full_name} userId={session.user.id} pageTitle="Statistikat" pageIcon="📊" package={profile.package_type}>
      <AdminStatsClient
        stats={{ totalUsers, totalCompanies, totalApplications, totalOffers, totalReviews, activeApplications, acceptedOffers, verifiedCompanies, newUsersMonth, newAppsMonth, newOffersWeek, premiumUsers,
