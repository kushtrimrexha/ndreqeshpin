...
export default function CompanyOffersClient({ offers }: { offers: Offer[] }) {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize:14, color:'rgba(232,234,240,0.45)' }}>Gjurmo statusin e çdo oferte të dërguar</p>
      </div>

      {/* Stats row */}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:28 }}>
        {[
          { label:'Gjithsej',  val: offers.length,        col:'#e8621a', icon:'💼' },
          { label:'Në pritje', val: counts.pending,        col:'#fbbf24', icon:'⏳' },
          { label:'Pranuar',   val: counts.accepted,       col:'#22d3a5', icon:'✅' },
          { label:'Fituar (€)',val: `€${totalEarned.toLocaleString()}`, col:'#60a5fa', icon:'💰' },
        ].map((s,i) => (
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:18, overflow:'hidden' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 100px 90px 90px 110px', gap:12, padding:'12px 20px', borderBottom:'1px solid rgba(255,255,255,0.07)', fontSize:11, fontWeight:700, color:'rgba(232,234,240,0.3)', textTransform:'uppercase' as const, letterSpacing:'0.07em' }}>
            <span>Projekti</span><span style={{ textAlign:'center' as const }}>Çmimi</span><span style={{ textAlign:'center' as const }}>Kohëzgjatja</span><span style={{ textAlign:'center' as const }}>Statusi</span><span style={{ textAlign:'right' as const }}>Data</span>
          </div>
          {filtered.map((o, i) => {
            ...
            return (
              <div key={o.id} className="offer-row"
                ...
                <div>
                  ...
                  {o.description && (
                    <p style={{ fontSize:12, color:'rgba(232,234,240,0.35)', marginTop:5, overflow:'hidden', display:'-webkit-box', WebkitLineClamp:1, WebkitBoxOrient:'vertical' as any }}>{o.description}</p>
                  )}
                </div>
                <div style={{ textAlign:'center' as const }}>
                  <span style={{ fontFamily:"'Fraunces',serif", fontWeight:900, fontSize:'1.1rem', color:'#e8621a' }}>€{o.price.toLocaleString()}</span>
                </div>
